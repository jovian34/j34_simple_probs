...
class ListOps():
    ...
    def __init__(self, length, total):
        self.length = length
        self.total = total
        temp_list = []
        temp_value = [0, 1]
    ...
    def _is_valid(self, num_list):
        for i in range(1, len(num_list)):
            if abs(num_list[-(i)]) > 10 and abs(num_list[-(i) -1]) > 0:
                return False
            elif num_list[0] == 0 and num_list[1] == -12:
                return False
        ...
        for i in range(len(self.final_list)):
            for j in range(len(self.final_list[i])):
                if self.final_list[i][j] == 0:
                    ...
                    elif j == 1 and self.final_list[i][0] == 0:
                        print('', self.final_list[i][j], ' ', end = '')
                    else:
                        print('+', self.final_list[i][j], ' ', end = '')
                elif self.final_list[i][j] < 0:
...
def get_inputs():
    length = input('Enter the number of integers between 1 and 9: ')
    length = int(length)
    total = input('Enter the integer value you wish to total to: ')
    total = int(total)
    values = [length, total]
    return tuple(values)

def main():
    length, total = get_inputs()
    list_obj = ListOps(length, total)
    list_obj.run()    
